...
export class ChatboxHamburgerButton extends RisuButton<ChatboxHamburgerButtonConfig> {
	constructor(
		id: string,
		options: Partial<ChatboxHamburgerButtonConfig> = {},
	) {
		super(id, options);
	}
	...
	protected override getButtonElement(): HTMLElement {
		...
		btnFamily.className =
			"flex items-center cursor-pointer lg:hover:text-green-500 svelte-ba8msf";
	...
	protected override onDocumentChange(btn: HTMLElement): () => void {
		return () => {
			...
			if (menu) {
				...
				else menu.lastElementChild!.after(btn);
			}
		};
	}
}
